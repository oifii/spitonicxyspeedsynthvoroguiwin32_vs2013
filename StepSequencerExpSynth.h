 ...
 *
 */
#ifndef StepSequencerExpSynth_h
#define StepSequencerExpSynth_h
...
class StepSequencerExpSynth : public Synth{

public:
	StepSequencerExpSynth(){
		...
		//ControlGenerator bpm = addParameter("tempo", 150).min(50).max(300);
		...
		for (int i = 0; i < NUM_STEPS; i++){
			//ControlGenerator pitchForThisStep = addParameter("step" + INT_TO_STRING(i) + "Pitch", randomFloat(30, 50)).min(30).max(50);
			ControlGenerator pitchForThisStep = addParameter("step" + INT_TO_STRING(i) + "Pitch", randomFloat(24, 40)).min(24).max(40);
			//ControlGenerator pitchForThisStep = addParameter("step" + INT_TO_STRING(i) + "Pitch", randomFloat(0, 10)).min(0).max(10);
		...
		*/
		...
		//Generator amplitude = ADSR(0.01, 0.1, 0, 0).trigger(metro) * 0.3; //original
		//Generator amplitude = ADSR(0.01, 0.1, 3, 0).trigger(metro) * 0.3; //xznn
		Generator amplitude = ADSR(0, 0.1, 3, 0).trigger(metro) * 0.3; //xznn
...
#pragma message("C++11 Support not available - StepSequencerExpSynth disabled")

#endif
	}
};

//TONIC_REGISTER_SYNTH(StepSequencerExpSynth)
